...
- [ ] **(TODO-nhẹ, chưa làm)** `completeBossVictory` chọn bonus NGẪU NHIÊN trong mảng `bossCompletionBonusNP` thay vì theo đúng index của Boss Card vừa đánh (server không biết `bossId` nào ứng với "dễ/trung bình/khó") — số hiển thị trên Boss Card trước trận và số thực nhận sau trận có thể lệch nhau. Cần truyền `bossId`/độ khó qua khi gọi `completeBossVictory` để khớp đúng — thuộc phạm vi Task #36 (Boss Card + rút 5/20 câu) nên chưa xử lý ở đợt này.

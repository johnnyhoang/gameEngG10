...
## Đợt cập nhật 2026-07-11 (3): Chân Khí v2 theo SUB_SPEC_ENERGY.md (Task #37)

Bối cảnh: triển khai `SUB_SPEC_ENERGY.md` (bản đề xuất v1) — Viện Chủ xác nhận qua AskUserQuestion chốt theo phương án Recommended cho cả 3 câu hỏi mở: (1) giữ đúng bộ số đề xuất (ải thường -30, Boss -100/=maxEnergy nếu maxEnergy<100, minigame Sơn Trang -10, Gatekeeper/đọc/chăm heo = 0), (2) Phương án A — Reset trọn gói (cạn 0 mới đếm giờ, hết giờ hồi ĐẦY 1 lần, KHÔNG hồi nhỏ giọt), (3) mốc cấu hình mặc định maxEnergy=100 (50-300)/resetHours=3 (∈{2,3,5}). Phát hiện xung đột kiến trúc thật giữa lúc phân tích — hỏi lại và Viện Chủ chọn rõ: `maxEnergy`/`resetHours` phải là cấu hình **riêng từng con** (đúng spec), không dùng chung toàn hệ thống như kiến trúc cũ (`ge10_game_settings` global).

- [x] **Data model:** `types/game.ts` — `PlayerProfile` thêm `maxEnergy` (riêng từng con), `resetHours: 2|3|5`, `energyDepletedAt: number|null` (mốc cạn 0 để tính giờ hồi); `GameSettings.maxEnergy` xóa hẳn (không còn global). `schema.sql` — `ge10_player_profiles` thêm cột `max_energy`/`reset_hours`/`energy_depleted_at`, đổi default `energy` từ 1000→100, clamp 1 lần các hồ sơ cũ có `energy > max_energy` xuống đúng trần mới, xóa key `max_energy` khỏi `ge10_game_settings` (không còn ai đọc).
- [x] **Store — regen "Reset trọn gói":** `createPlayerSlice.ts` — `useEnergy` ghi `energyDepletedAt = Date.now()` khi trừ chạm đúng 0; `addEnergy` đọc `player.maxEnergy` (không còn `gameSettings.maxEnergy`) + hủy `energyDepletedAt` ngay khi có năng lượng trở lại (dù chưa đầy); action mới `tickEnergyRegen()` — nếu đã cạn đủ `resetHours` thì hồi ĐẦY 1 lần về `maxEnergy`; `checkDailyReset` luôn hồi đầy theo `player.maxEnergy` qua ngày mới bất kể trạng thái (đúng ghi chú spec §5). `tickEnergyRegen` được gọi ở 3 điểm: đầu `useEnergy` (tự-heal trước khi tiêu), `App.tsx` lúc mount + trong interval 5 phút sẵn có, và `TopHUD.tsx` interval riêng 30 giây (mở khóa UI kịp thời không cần reload).
- [x] **Chi phí hoạt động:** `Arena.tsx` — Boss cost đổi từ hardcode `energy: 100` sang `bossEnergyCost = Math.min(100, player.maxEnergy ?? 100)` (đúng luật "maxEnergy<100 thì tốn=maxEnergy, luôn đánh được 1 lượt Boss khi đầy bình"). `RelaxationZone.tsx` — mini-game Sơn Trang trước đây HOÀN TOÀN MIỄN PHÍ dù có thưởng NP/XP (vi phạm nguyên tắc cốt lõi "sinh điểm thì tiêu Chân Khí" ở §1); thêm cost -10/ván cho mọi game trừ "Sơ Đồ Ôn Tập" (đã tự ghi chú "Không điểm — luyện trí nhớ" nên giữ miễn phí đúng luật), card khóa mờ + tooltip giờ hồi khi thiếu năng lượng.
- [x] **Cấu hình riêng từng con (UI + backend):** `StudentProfileView.tsx` — thêm panel "⚙️ Cấu Hình Riêng Cho Con Này" (input Trần Chân Khí 50-300 + select giờ hồi 2/3/5h) cạnh khối "Nạp Chân Khí" sẵn có, gọi action mới `adminSetEnergyConfig`. `SettingsManager.tsx` (Thiên Cơ Các, global) — bỏ hẳn input "Năng lượng tối đa" cũ (đã chuyển thành cấu hình per-con), cập nhật câu mô tả chỉ dẫn sang tab Thân Phận. `createAdminSlice.ts`/`store/types.ts` — action mới `adminSetEnergyConfig(studentId, maxEnergy, resetHours)`. `server.ts` — endpoint mới `POST /api/admin/set-energy-config` (permission qua `checkStudentManagementPermission` action `set_energy_config`, secondary_parent bị chặn giống `refill_energy`); endpoint cũ `POST /api/admin/refill-energy` sửa lại đọc `max_energy` của CHÍNH học sinh đó thay vì `loadMaxEnergy()` global (nếu không sửa sẽ tính sai % dựa trên trần cũ 1000 đã không còn tồn tại); `PUT /api/admin/game-settings` bỏ nhánh `maxEnergy`; xóa hẳn 2 hàm chết `loadMaxEnergy`/`saveMaxEnergy`. `POST /api/profile/:id/sync` — thêm `energy_depleted_at` vào upsert + merge-conflict theo `server_updated_at` (giống `energy`), CỐ Ý không đồng bộ `max_energy`/`reset_hours` từ client (chỉ phụ huynh chỉnh qua endpoint riêng, con tự sync không được ghi đè).
- [x] **UI hiển thị:** `TopHUD.tsx` — bỏ hardcode `/1000`, đọc `player.maxEnergy`; khi cạn 0 hiện "Hồi lúc HH:mm" màu đỏ thay vì số 0/X vô nghĩa. `PetStableOverlay.tsx` — thêm trigger `energy-depleted` (§2.5 danh sách trigger heo), ưu tiên cao nhất trong vòng lặp kiểm tra (không chờ "30 phút sau đăng nhập" như idle/hunger vì đây là thông báo cần thiết ngay), vẫn tôn trọng cooldown chống-spam 30 phút dùng chung + các guard `isOpen`/`isDungeonScreen`/`hasOtherModal` sẵn có; nội dung thoại nêu rõ giờ hồi tính từ `energyDepletedAt + resetHours`.
- [x] **Verify:** `npm run build` (frontend) và `cd backend && npm run build` đều xanh. Grep xác nhận không còn `gameSettings.maxEnergy`/`gameSettings?.maxEnergy` sót lại ở `src/` (kể cả nhánh dead-code `ENERGY_RESTORED` event cũ trong `createUISlice.ts` cũng đã trỏ sang `player.maxEnergy`).
- [ ] *(Ghi nhận, không thuộc phạm vi Task #37)* Chưa thêm trạng thái "disable + tooltip" trực quan cho các thẻ Arena (practice/survival/revenge/boss) khi hết năng lượng — hiện vẫn chỉ chặn bằng `toast.error` lúc bấm (hành vi có từ trước, không phải regression). RelaxationZone đã có xử lý disable+tooltip đầy đủ vì đang sửa cùng lúc; Arena nên làm tương tự ở đợt sau nếu Viện Chủ muốn đồng bộ UX.

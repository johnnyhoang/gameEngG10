...
- **🔐 Bảo mật PIN đa quản trị (Multi-Admin PIN Authentication):**
  - Bãi bỏ cơ chế dùng chung 1 mã PIN gia đình. Mỗi tài khoản quản trị (Phụ huynh Chính, Phụ huynh Phụ, Phó Viện, Viện Trưởng) tự thiết lập **mã PIN bảo mật cá nhân riêng** (4-6 chữ số, mã hóa bcrypt).
  - Khi thực hiện các thao tác nhạy cảm (duyệt đổi quà, nạp năng lượng, thay đổi cài đặt), hệ thống yêu cầu nhập mã PIN cá nhân của chính profile đó để xác thực.
- **Rời Nhóm:**

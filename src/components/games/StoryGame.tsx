...
export const StoryGame: React.FC = () => {
  ...
  const handleStoryAnswer = (selectedOption: string) => {
    ...
    if (selectedOption.trim().toLowerCase() === correctAnsStr.trim().toLowerCase()) {
      ...
      else if (storyStep === 3) { setStoryInventory(prev => [...prev, '📜 Cuộn Sách Cổ']); setStoryStep(4); awardCoinsAndXp(60, 50, 'Phiêu lưu tình huống', 'Giải thoát Heo Maikawaii thành công'); }
    } else {

...
import { Crown, Shield, Users, User, GraduationCap, Link as LinkIcon } from 'lucide-react';
...
export const OrgChart: React.FC<OrgChartProps> = ({
  ...
  return (
    <div className="space-y-6 py-2 overflow-x-auto min-w-full">
      ...
      <div className="flex flex-col items-center justify-start space-y-5 min-w-[700px] max-h-[68vh] overflow-auto p-4 border border-white/5 bg-slate-950/20 rounded-2xl">
            ...
            {admins.map(admin => {
              ...
              return (
                <div key={admin.id} className="relative group">
                  ...
                  {/* CSS Hover Tooltip */}
                  <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 w-48 p-2.5 bg-slate-900/95 border border-synth-magenta/30 rounded-lg text-[9px] text-slate-300 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-50 shadow-2xl backdrop-blur-md space-y-0.5">
        ...
        {/* ĐƯỜNG NỐI DỌC 1 */}
        {viceAdmins.length > 0 && (
          <div className="w-0.5 h-5 bg-gradient-to-b from-synth-magenta/50 to-purple-500/50"></div>
        )}
              ...
              {viceAdmins.map(vice => {
                ...
                return (
                  <div key={vice.id} className="relative group">
                    ...
                    {/* CSS Hover Tooltip */}
                    <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 w-48 p-2.5 bg-slate-900/95 border border-purple-500/30 rounded-lg text-[9px] text-slate-300 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-50 shadow-2xl backdrop-blur-md space-y-0.5">
        ...
        {/* ĐƯỜNG NỐI DỌC 2 */}
        <div className="w-0.5 h-5 bg-gradient-to-b from-purple-500/50 to-synth-cyan/50"></div>
        ...
        <div className="w-full space-y-6">
          ...
          <div className="flex flex-wrap justify-center gap-6 px-2">
            {primaryTeachers.map(primary => {
              ...
              return (
                <div key={primary.id} className="glass-panel border border-white/5 rounded-2xl p-4 bg-white/2 space-y-4 flex flex-col items-center relative hover:border-synth-cyan/30 transition-colors max-w-sm min-w-[280px]">
                  ...
                  <div className="flex flex-col items-center space-y-2 w-full">
                    ...
                    <div className="flex items-center justify-center gap-4 w-full relative">
                      ...
                      {/* Đường nối ngang nếu có Phó Chủ Nhiệm */}
                      {secondaries.length > 0 && (
                        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-4 h-0.5 bg-dashed bg-orange-500/40 flex items-center justify-center">
                          <LinkIcon className="w-2.5 h-2.5 text-orange-400" />
                        </div>
                      )}
                  ...
                  {/* Đường nối dọc xuống Sĩ tử */}
                  <div className="w-0.5 h-4 bg-synth-cyan/30"></div>
        ...
        {freeStudents.length > 0 && (
          <div className="w-0.5 h-6 bg-dashed bg-slate-600/50"></div>
        )}

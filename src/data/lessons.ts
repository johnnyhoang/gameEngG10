...
GRADE9_LESSONS.forEach(lesson => {
  const textbook = enrichTextbookAttributes(lesson.id, lesson.category, lesson.subject);
  clonedLessons.push({
    ...lesson,
    gradeTier: 9 as const,
    loai: lesson.loai ?? textbook.loai,
    bai: lesson.bai ?? textbook.bai,
    hamNguyenTo: lesson.hamNguyenTo ?? textbook.hamNguyenTo
  });
});
...
([6, 7, 8, 10, 11, 12] as GradeTier[]).forEach(tier => {
  GRADE9_LESSONS.forEach(lesson => {
    let subId = lesson.subject;
    
    // Tách môn THPT cho Lớp 10, 11, 12 (đề phòng sau này có lessons cho science/history_geography)
    if (tier >= 10 && tier <= 12) {
      if (subId === 'science') {
        if (lesson.id.startsWith('sci-phy-')) subId = 'physics';
        else if (lesson.id.startsWith('sci-chem-')) subId = 'chemistry';
        else if (lesson.id.startsWith('sci-bio-')) subId = 'biology';
      } else if (subId === 'history_geography') {
        if (lesson.id.startsWith('his-')) subId = 'history';
        else if (lesson.id.startsWith('geo-')) subId = 'geography';
      }
    }
    
    const textbook = enrichTextbookAttributes(lesson.id, lesson.category, subId);
...
// Sinh tự động bài giảng mẫu (mock lessons) cho 21 môn học Đại học - CS (Lớp 13)
...
const csMockLessons: Lesson[] = CS_SUBJECTS_IDS.map((subId, index) => {
  const lessonId = `cs-mock-lesson-${subId}`;
  const textbook = enrichTextbookAttributes(lessonId, `cs-topic-${index}`, subId);
  return {
    id: lessonId,
    subject: subId,
    gradeTier: 13,
...
## 1. Mục tiêu môn học
- Hiểu và áp dụng các khái niệm cơ bản của môn học vào thực tế.
- Xây dựng tư duy giải quyết vấn đề hệ thống máy tính & robot.

## 2. Lộ trình tu học
- Tuần 1-4: Cơ sở lý thuyết và các khái niệm nền tảng.
- Tuần 5-8: Thực hành chuyên sâu và lập trình ứng dụng.
- Tuần 9+: Đồ án môn học & Đánh giá năng lực.`
  };
});

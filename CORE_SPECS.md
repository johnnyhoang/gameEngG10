...
*   **Cơ Chế "Thú Về Chuồng" (Pet Stable & Overlay)**:
    ...
    *   **Các thời điểm heo xuất hiện** (dưới dạng **overlay đè lên layout hiện tại**, nền phía sau làm mờ nhẹ để tập trung vào heo):
        ...
        2.  *Gác Cổng lúc đăng xuất (Logout Gate):* Khi thiếu hiệp bấm Thoái Ẩn (Đăng xuất), Heo xuất hiện mang theo một trang Cẩm Nang khác để dặn dò trước khi rời giang hồ. Hành động đăng xuất chỉ thực sự hoàn tất **sau khi** heo được cho về chuồng (xem quy tắc dismiss chung bên dưới).
    ...
    *   **Cách duy nhất cất heo về chuồng — áp dụng đồng bộ cho MỌI trigger, kể cả Gác Cổng:** overlay **KHÔNG có nút đóng (X)** và **KHÔNG có nút xác nhận riêng kiểu "Đã Lĩnh Ngộ"**. Thiếu hiệp bắt buộc phải *cho ăn*, *tương tác* (vuốt ve/khen) hoặc *thọt lét (Tickle)* thì heo mới chịu về chuồng — kể cả lúc đăng nhập/đăng xuất. Thọt lét là lối thoát miễn phí không tốn tài nguyên; cho ăn vẫn tốn NP + XP theo đúng quy tắc hiện hành ở mục trên (chấp nhận rủi ro tụt cấp).
    *   **Chống dồn dập (anti-spam):** Các trigger tự động (idle/đói) dùng chung một cooldown tối thiểu 10 phút giữa các lần bật overlay tự động — dismiss vì lý do này không cho phép lý do khác bật lại overlay ngay sau đó.
    *   **Kỷ luật thi cử — không phá bĩnh giữa trận:** Khi thiếu hiệp đang trong trận đấu (Đấu Trường / Boss / đang làm bài luyện tập), mọi trigger tự động (idle/đói) bị **hoãn lại**, dồn tích và chỉ hiện ra ngay khi thiếu hiệp thoát về Sơn Trang/Bản Đồ. Trigger triệu hồi thủ công vẫn bị khóa trong lúc làm bài như hiện tại.
...
*   **Điểm chạm tương tác của Thiếu Hiệp:**
    *   *Gác Cổng qua Heo Maikawaii (Login/Logout) — xem §2.5 "Thú Về Chuồng":* Trang cẩm nang không còn tự hiện độc lập bằng một modal sách riêng kèm nút "Đã Lĩnh Ngộ" nữa. Thay vào đó, một trang ngẫu nhiên được **Heo Maikawaii mang ra trình bày ngay trong overlay Sân Nuôi Thú** mỗi khi đăng nhập và mỗi khi đăng xuất. Cách duy nhất để đóng overlay này (và hoàn tất đăng xuất, nếu là mốc logout) là cho ăn/tương tác/thọt lét heo — không còn nút xác nhận đọc riêng.
    *   *Thân Phận (Profile Page):* Thiếu hiệp có thể mở toàn bộ cuốn cẩm nang (giao diện sách đầy đủ, lật từng trang/chương) để tra cứu quy tắc bất kỳ lúc nào — trải nghiệm "cuốn sách" đầy đủ này giữ nguyên như cũ, không đổi.
*   **Quyền quản trị của Viện Chủ:** Tại **Ngân Các** (hoặc bảng quản trị), Viện Chủ có thể tra cứu toàn bộ cuốn sách và có một biểu mẫu riêng để **nạp thêm các trang dặn dò** (Quy định riêng của gia đình, nhắc nhở ôn bài, lịch học thực tế) vào cuốn cẩm nang của thiếu hiệp. Các trang mới này tự động được đưa vào vòng random mà Heo mang ra ở mốc đăng nhập/đăng xuất. Dữ liệu các trang mới này được đồng bộ và lưu trữ tự động.
...
| 🏮 **Bách Hóa Phường** | 💊 Quầy Linh Đan & Bùa Chú <br> 👘 Tiệm Y Phục (Themes) <br> 🎁 Quầy Phúc Lợi Gia Môn | Khai Ngộ Quyển, Hồi Nguyên Đan... <br> Đào Hoa, Tinh Không, Trúc Lâm... <br> Từng món quà do Viện Chủ set up. |
...
- Độ khó Gatekeeper: `difficulty` từ 3–5 (trung bình), không quá dễ (≤2) cũng không quá khó (≥8).
- Mỗi câu Gatekeeper chỉ dùng dạng **MCQ hoặc Wordform** — không dùng dạng tự luận dài.

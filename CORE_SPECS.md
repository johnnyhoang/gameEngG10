...
### 2.8.1 Cấp 1 — Các Page gốc
Trùng với các module chính/phụ đã có ở §2.1: **Bản Đồ (WorldMap)**, **Đấu Trường**, **Hang Luyện Công**, **Sơn Trang Thư Giãn**, **Bách Hóa Phường**, **Sân Thú Nuôi**, **Thân Phận**.

### 2.8.2 Cấp 2 — Phân loại Vùng theo từng Page Cấp 1

#### A. Bản Đồ Lớn (WorldMap) → 3 loại Vùng
Bản đồ được xem như một vùng đất rộng lớn cần khám phá, gồm nhiều khu vực quy về 3 loại:

| Loại vùng | Ý nghĩa | Thành phần hiện tại map vào |
| :--- | :--- | :--- |
| 🏯 **Thánh Địa (Dungeon)** | Nơi thực hiện thử thách & cạm bẫy | Hang Luyện Công |
| ⚔️ **Đấu Trường (Arena)** | Nơi thi thố | Đấu Trường |
| 🌫️ **Mật Cảnh (Secret Realm)** | Nơi khám phá | Sơn Trang Thư Giãn, Bách Hóa Phường, Sân Thú Nuôi |

> ⚠️ *Điểm cần chốt lại với user:* Bách Hóa Phường và Sân Thú Nuôi không mang tính "thử thách/thi thố" nên tạm xếp chung vào Mật Cảnh (khám phá tự do) — đây là suy luận hợp lý nhất, không phải map 1-1 tường minh từ yêu cầu gốc, cần xác nhận lại trước khi triển khai.

#### B. Hang Luyện Công → 3 loại Vùng
...
#### C. Sơn Trang Thư Giãn → 3 Cảnh
Khác với A và B, 3 Cảnh ở đây **thuần túy trực quan/trang trí**, không phân biệt chức năng — toàn bộ 9 mini-game hiện có (Thẻ Nhớ, Ghép Cặp, Sơ Đồ Ôn Tập, Tình Huống RPG, Du Khảo Kỳ Thú, Trình Tự Giải, Đọc Hiểu Sâu, Giảng Cho AI, Ghép Sơ Đồ) về bản chất đều là "khu vui chơi trí tuệ" như nhau, chỉ đổi giao diện/khung cảnh bao quanh:
...
### 2.8.4 Cấu trúc dữ liệu Page & Dấu vết Khám phá (đề xuất, chưa triển khai)

```typescript
type PageLevel = 1 | 2 | 3;

type ZoneTypeWorldMap = 'thanh_dia' | 'dau_truong' | 'mat_canh';
...
interface MapPage {
  id: string;
  level: PageLevel;
  parentId: string | null;        // null nếu là Cấp 1
  rootPageId: string;              // Page Cấp 1 gốc (worldmap/hang/sontrang/...)
  zoneType?: ZoneTypeWorldMap | ZoneTypeHangLuyenCong | ZoneTypeSonTrang; // chỉ có ở Cấp 2
  name: string;
  icon: string;
}

// Dấu vết khám phá — ghi theo từng học sinh, áp dụng chủ yếu cho Page Cấp 2 (và Cấp 3 khi đã định nghĩa)
...
### 2.8.5 Việc cần chốt tiếp (chưa làm ngay)
1.  Định nghĩa Page Cấp 3 cho từng nhánh Đấu Trường / Sơn Trang / Bách Hóa Phường / Sân Thú Nuôi.
2.  Quy tắc gán cố định Chuyên đề → Hầm nguyên tố (Hỏa/Băng/Thạch).
3.  Bách Hóa Phường & Sân Thú Nuôi có thật sự cần gán vào loại vùng Mật Cảnh, hay nên tách riêng ngoài 3 loại vùng của WorldMap?
4.  Thiết kế bảng PostgreSQL cho `MapPage` (dữ liệu tĩnh, seed 1 lần) và `PageExplorationState` (dữ liệu động theo từng học sinh).

...
| :--- | :--- | :--- | :--- |
| Role quản trị | Hiệu Trưởng → **Viện Trưởng**; Hiệu Phó → **Phó Viện Trưởng** | `SUB_SPEC_FAMILY_ROLE.md`, `backend/src/helpers/profileAdapter.ts`, `backend/src/routes/admin.ts`, `backend/src/routes/family.ts`, `src/utils/roleHelpers.ts`, `TutorConsole*`, `ProfileSelectionScreen.tsx`, `TopHUD.tsx` | Display + docs; giữ `truong_vien`, `pho_vien` |
...
| Quản trị | Bảng Quản Trị → **Phòng Điều Hành**; Chính Điện → **Sổ Danh Bộ**; Thiên Cơ Các → **Phòng Học Vụ**; Vạn Quyển Các → **Kho Đề Thi**; Ngân Các → **Phòng Tài Vụ** | `TutorConsole.tsx`, `SettingsManager.tsx`, `QuestionBankManager.tsx`, `RewardManager.tsx`, handbook và sub-spec role/energy | Display/navigation; giữ tab IDs |
